    ...
    try:
        ...
        for i in range(0,len(chunks),batch_size):
            ...
            for i, chunk in enumerate(batch_items):
                # # 假设原始 state["chunks"] 是这样的：
                # state["chunks"] = [
                #     {"item_name": "手机", "content": "华为手机"},
                #     {"item_name": "充电器", "content": "快充充电器"}
                # ]
                #
                # # 如果不拷贝直接修改：
                # batch_items = state["chunks"][0:2]  # 切片得到新列表，但字典是引用
                # batch_items[0]['dense_vector'] = [0.1, 0.2, ...]
                # # ⚠️ state["chunks"][0] 也被修改了！
                #
                # # 如果后续代码还依赖原始的 state["chunks"]（没有向量字段），就会出问题，所以要copy
